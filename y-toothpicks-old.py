...
def tssc(x):
    ...
    return tss_caged(k + 1, p + x) - tss_caged(k, cg)
...
def c_caged(k, x):
    ...
    return c_caged(k - 1, x) +\
           tss_caged(k - 1, corner_rem) +\
           si_caged(k - 2, corner_rem - 1) -\
           tss_caged(k - 2, component_rem) +\
           c_caged(k - 2, component_rem)
...
def i_caged(k, x):
    ...
    return i_caged(k - 1, x) + ls_caged(k - 1, ls_rem) - ls_caged(k - 2, ls_rem) + ls_caged(k - 1, ls_crem) - ls_caged(k - 2, ls_crem) + (c_caged(k - 2, component_rem - 1) + (component_rem > 0)) + si_caged(k - 2, component_rem) - (x > 2 ** k)
...
def tssg(x):
    return tss_caged(smallest_pow2_geq(x), x)


def ag(x):
    return tss_to_a(tssg(x), x)
